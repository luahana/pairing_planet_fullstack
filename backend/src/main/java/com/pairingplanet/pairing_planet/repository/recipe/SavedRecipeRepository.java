...
public interface SavedRecipeRepository extends JpaRepository<SavedRecipe, SavedRecipeId> {
    ...
    @Query("SELECT sr FROM SavedRecipe sr JOIN FETCH sr.recipe r WHERE sr.userId = :userId AND r.isDeleted = false ORDER BY sr.createdAt DESC, sr.recipeId DESC")
    ...
    @Query("SELECT sr FROM SavedRecipe sr JOIN FETCH sr.recipe r WHERE sr.userId = :userId AND r.isDeleted = false " +
