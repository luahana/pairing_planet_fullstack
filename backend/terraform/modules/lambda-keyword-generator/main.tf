# =============================================================================
# LAMBDA KEYWORD GENERATOR MODULE
# Generates multilingual search keywords for foods_master using Google Gemini AI
# =============================================================================

locals {
  function_name = "${var.project_name}-${var.environment}-keyword-generator"
...
resource "aws_iam_role" "lambda_execution" {
  name = "${var.project_name}-${var.environment}-keyword-generator-lambda-role"
...
resource "aws_iam_role_policy" "secrets_access" {
  name = "${var.project_name}-${var.environment}-keyword-generator-secrets-policy"
...
# Triggers Lambda every 6 hours to process foods needing keywords
# -----------------------------------------------------------------------------
resource "aws_cloudwatch_event_rule" "schedule" {
  name                = "${var.project_name}-${var.environment}-keyword-generator-schedule"
  description         = "Trigger keyword generator Lambda on schedule"
...
resource "aws_cloudwatch_event_target" "lambda_target" {
  ...
  target_id = "KeywordGeneratorLambda"
  arn       = aws_lambda_function.keyword_generator.arn
}

resource "aws_lambda_permission" "eventbridge" {
  statement_id  = "AllowEventBridgeInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.keyword_generator.function_name
